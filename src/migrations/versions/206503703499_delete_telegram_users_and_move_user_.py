...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("api_key", sa.String(length=64), nullable=True)
    )
    op.add_column(
        "users", sa.Column("max_tunnels", sa.Integer(), nullable=True)
    )
    ...
    connection.execute(
        sa.text("UPDATE users SET max_tunnels = 1 WHERE max_tunnels IS NULL")
    )

    op.alter_column(
        "users", "api_key", existing_type=sa.String(length=64), nullable=False
    )
    op.alter_column(
        "users", "max_tunnels", existing_type=sa.Integer(), nullable=False
    )
